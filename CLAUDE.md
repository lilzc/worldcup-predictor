...
- **平局生成与表达问题（收敛病灶，2026-07-03合并三条挂起项）**：三个现象同源，解封条件统一，届时立**单个spec**一起评估，不分开修。
  ...
  **解封条件（沿用GSV追踪器门槛）**：正式OOS段（7-03起）GSV触发场次≥10且DC无水假想ROI显著为正（**口径=触发即下**，有赔率即计入，无edge门槛，与追踪器代码一致；不得事后切换口径）。**样本外起点=2026-07-03（猜想记档日），严格晚于猜想成形；2026-06-29~07-02淘汰赛为灰带cohort（grey），入追踪器单独段观察，不计入正式OOS N，不参与解封裁决。** 届时按优先级：先裁①③平局参数，后裁②DC出口，三件事同一spec不分开动。
...
| GSV AH+0.5 ROI | **+115.5%** (6注，真实赔率结算) | **已审计：不可持续**（样本期冷门密集子集）。Turkey AH+0.5 WIN已正确计入(Turkey away +0.5覆盖)。解封须正式OOS段(7-03+)≥10场且DC ROI显著为正；6-29~7-02灰带不计入 |
