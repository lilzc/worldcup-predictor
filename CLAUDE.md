...
## 会话与上下文纪律（2026-07-05 立项）

- **每个比赛日收盘（结算+报告完成）后跑 `/context-save` 并开新会话**，新会话从保存的上下文恢复。禁止单会话跨多个比赛日。
- **理由（事故台账）**：主会话曾单会话跑 15 天、触发 53 次上下文压缩，每次压缩后出现行为漂移——推单两轮输出不一致、编造幻觉比赛、漏推错推，均集中发生在压缩之后。
- 探索性/批量分析一律写成 `scripts/` 下的落盘脚本（或委派子代理、只回摘要），不允许内联 heredoc 在主会话直接跑——这是压缩频率的最大来源，也违反「所有正式输出必须可复现」原则。
...
**odds-api 日期漂移（2026-07-04 固化）**：odds-api 的 `commence_time` 对 UTC 晚场存在 +1 日漂移，3 例实证（USA/Bosnia 7-01→7-02、Switzerland/Algeria 7-02→7-03、Colombia/Ghana 7-03→7-04）。已以两项规则消化，无需人工干预：① `fetch_today_matches` 改为 +30h 窗口（今日 00:00 → 明日 06:00 UTC），漂移场次不再被漏拉；② `daily_sync` 新增 `DATE_DRIFT_DUP` 自动归档（比分+对阵一致+日期差1天+已在DB），不再进入 `pending_results.jsonl`。赛程日期权威来源以 martj42 / 维基百科为准，odds-api 日期仅供配额查询。
...
- **group_stage 波动罚分作用域修复（2026-07-10，对抗审查#1，走 factor-workflow）**：`group_stage_volatility`（adjustments.py）对 Elo>1850 强队扣 0.12 胜率灌入平局，本意仅针对小组赛轮换（back-test 依据 Brazil 1-1 Morocco / Spain 0-0 Cape Verde 均为小组赛），但 `is_group_stage` 参数全仓库无一处传 False → 罚分被误施到淘汰赛。修复：`predict()`/`walkforward._build_mat_custom` 新增 `match_date`，`date>=KNOCKOUT_START(2026-06-28)` 时 `is_group_stage=False`（KNOCKOUT_START 迁至 config.py 为唯一真源）。**验证**：24 场真实淘汰赛 1X2 Brier 0.4010→0.3148（−0.086 改善），强队胜率恢复 +0.11~0.16 且实际多数获胜；小组赛零回归（walkforward 逐字相同，MATCHES_ODDS 全为 06-15~06-27 小组赛）。**这是"40-60%强队胜率低估"的机制根因之一——非分布问题，是罚分误施。** 默认 `match_date=None→按小组赛(保守)`；诊断路径 `today.py:303 _compute_gsv_legacy_dc_edge`（喂已挂起 DC 猜想追踪）有意保留默认+注释，不静默。backtest.py 未接线（含前视非基准，测不出此 fix，属预期）。见 specs/knockout_group_stage_penalty_fix.md。
  - **配线补全（2026-07-23 follow-up）**：07-10 修复把 `match_date` 通到 `_build_mat_custom` 并算出 `_is_group`，但 **production 分支**（walkforward.py:388）的 `apply_all` 漏消费该变量、取默认 True——`_is_group` 当时只在 legacy 分支（line 376）用到。已补 `is_group_stage=_is_group`，与 legacy 分支和 predict.py 对齐。**验证（no-op）**：现数据下修复前后 walkforward 输出逐字节相同（Edge ROI +53.2% 32注不变）；MATCHES_ODDS 里 5 场 06-28 key 是 odds-api +1日漂移，循环用 results DB 权威日期 06-27（小组赛末轮）推 `_is_group`，故全 42 场 `_is_group=True`，修复为空转。价值：补断掉的配线，未来加入权威淘汰赛日期场次或转用下届赛事时 production 分支才不会重新吃罚分 bug。回归测试 tests/test_walkforward_group_stage.py（red-first 实证）。见 specs/walkforward_group_stage_wiring_fix.md。
...
- **平局生成与表达问题（收敛病灶，2026-07-03合并三条挂起项）**：三个现象同源，解封条件统一，届时立**单个spec**一起评估，不分开修。
  ...
  **解封条件（沿用GSV追踪器门槛）**：正式OOS段（7-03起）GSV触发场次≥8且DC无水假想ROI显著为正（**口径=触发即下**，有赔率即计入，无edge门槛，与追踪器代码一致；不得事后切换口径）。**解封阈值N≥8为2026-07定档条款，任何变更需显式讨论决策，不得以代码对齐名义修改。** **样本外起点=2026-07-03（猜想记档日），严格晚于猜想成形；2026-06-29~07-02淘汰赛为灰带cohort（grey），入追踪器单独段观察，不计入正式OOS N，不参与解封裁决。** 届时按优先级：先裁①③平局参数，后裁②DC出口，三件事同一spec不分开动。
  **⛔ 收官裁决（2026-07-23，WC2026 赛事结束）**：正式 OOS 定格 **N=3/8**（DC 无水假想 ROI +109.6%，14 注有赔率），赛事结束不再有新 GSV 触发场次，**N≥8 解封阈值确定无法达成**。**②GSV/DC出口猜想永久存档（样本耗尽，既未证实也未证伪）**——追踪器代码、gsv_shadow_log.jsonl 证据、上述解封条款全部保留供未来赛事复用，但本届不再解封、不开 DC 下注出口。**GSV_LAMBDA_FACTOR=0.80 参数本身不受影响**（它只覆盖 AH/OU 矩阵、有独立 walkforward O2.5 验证，与 DC 出口猜想是两回事，继续按参数边界管理）。**①③平局参数（DRAW_MIN_PROB/DRAW_MIN_EDGE）保持挂起**：淘汰赛平局样本已定格 8/32=25%，可择机做最终分区间统计，但无新数据来源、不在本届动。未来若模型复用到下届赛事，追踪器可重启计数、解封条款自动生效。
...
- **Turkey-USA 案例（第60场，2026-06-25 正式日期）：** GSV 触发（USA Elo 1869 **主场** home_adv=1.05，diff=158）。**✓ 主客修正（2026-07-03）**：原 MATCHES_ODDS 误写 Turkey 为主队（diff全负、赔率对调），修正后 USA=主场。walkforward 现在正确评估本场：推 OU 小3.0 @1.72，实际5球(2+3) → LOSE。1x2: USA胜40.8% vs 市场48.3%；平局32.5% vs 25.1%（两数字均以USA为主场基准，与双矩阵生产路径对齐）。Turkey DC edge=-7.5pp → 不触发DC追踪（负edge）；Turkey AH+0.5 WIN（Turkey作客赢/cover +0.5）。平局gap14.4%>8%被LOW拦截，零代价。**回归验证记录**：双路径一致性（today.py edge == walkforward production edge）是每场案例的标准验收项目。DC出口猜想解封条件：正式OOS段(7-03+)GSV触发≥8且DC假想ROI显著为正；6-29~7-02灰带观察不裁决。**Wikipedia核验（2026-07-03）**：维基百科 Group D 将本场列为"Turkey vs United States"（Turkey首位），与martj42/DB（USA首位）不一致。**代码验证（poisson.py line 113-114）**：`home_advantage = 1.05 if home_team in WC_HOST_NATIONS else 1.0`——只有 `home_team`（不是away_team）在 `{"USA","Canada","Mexico"}` 里才触发 1.05；对非host场次 home_team 仅是记账排序，不影响 λ。因此：若 Turkey=home → Turkey不在 WC_HOST_NATIONS → 1.0，USA在away位也是1.0，USA在本国主场得零加成；若 USA=home → USA∈WC_HOST_NATIONS → 1.05，符合设计意图。**决定：DB保持USA=home**，原因是 host 判定依赖 USA 居 home 位，而该场在美国本土举行（SoFi Stadium, LA），host 主场成立。"物理主场公约"措辞作废；规则是**东道主身份+home槽位同时满足才触发1.05，两者缺一不可**。85场全库比对零主客不一致（仅此场存在公约分歧）。**Host居away位抽验**：全库唯一一场=South Africa vs Canada (2026-06-28 Knockout，SoFi Stadium LA，neutral=TRUE)，Canada=away，代码给1.0；正确——该场在美国本土非加拿大主场，无设计缺口。
...
下表是补录06-23~06-29共32场缺口、DB去重（113→85条）后的干净基准，未来参数迭代必须对比此表：
...
| GSV AH+0.5 ROI | **+115.5%** (6注，真实赔率结算) | **已审计：不可持续**（样本期冷门密集子集）。Turkey AH+0.5 WIN已正确计入(Turkey away +0.5覆盖)。解封须正式OOS段(7-03+)≥8场且DC ROI显著为正；6-29~7-02灰带不计入 |
