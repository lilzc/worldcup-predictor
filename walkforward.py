...
# GSV 矩阵策略: "production"(默认) = 与 predict.py 对齐，GSV 只覆盖 AH/OU，1X2 用未压矩阵
#               "legacy" = 旧行为，GSV 应用于完整矩阵（含 1X2），仅供历史对比
WF_GSV_MODE = "production"
...
def _build_mat_custom(home: str, away: str, he: float, ae: float, ad_state: dict,
    ...
    # production 模式: 与 predict.py 双矩阵对齐
    # Step 1: 1X2 用非GSV矩阵
    mat = score_matrix(home, away,
                       custom_home_elo=he, custom_away_elo=ae,
                       custom_att_home=att_h, custom_def_home=def_h,
    ...
    if gsv_fired:
        ...
        for k, v in raw_gsv.items():
            if k.startswith("ah") or k.startswith("over") or k.startswith("under"):
                probs[k] = v
        # 返回 mat_gsv 供 _scan_from_probs 的 ou_prob/ah_prob fallback 使用
        return mat_gsv, probs, diff, gsv_fired
